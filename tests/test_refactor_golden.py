...
#1250 re-baselined `grooved_shaft`, `grid_plate` and `scattered_plate`: these fixtures carry a
required placement failure that was reported below error severity, so the automatic path
returned them as successes. The build now records a `plan_incomplete` error summarising the
loss. Checked against this corpus's own standard before re-recording — annotations identical,
labels identical, page identical, one build issue ADDED and none removed. The drawings did not
